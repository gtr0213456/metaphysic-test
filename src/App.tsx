...
// DataTag 組件（value 加大到 text-base）
...
export default function App() {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      alert("連線失敗：" + (e.message || JSON.stringify(e) || "未知錯誤，請檢查連線"));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        {data && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-12 duration-1000">
            ...
            <div className="bg-gradient-to-br from-indigo-900/40 to-transparent border border-indigo-500/30 rounded-[3rem] p-10 backdrop-blur-3xl relative overflow-hidden">
              ...
              <p className="text-2xl font-serif italic text-white mb-8 leading-relaxed">"{data.dailyAdvice || '無法生成建議'}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="col-span-full bg-black/40 p-4 rounded-2xl border border-white/5">
                    <p className="text-sm text-pink-200/70 leading-relaxed">
                      <span className="text-pink-500 font-bold">預警：</span>{data.relationship.warning || '無警示'}
                    </p>
                  </div>
                </div>
              </div>
